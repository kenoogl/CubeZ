...
!> *******************************
subroutine fileout_ikj (sz, g, s, dh, org, fname)
implicit none
integer                                                :: nn, ix, jx, kx, i, j, k, g
integer, dimension(3)                                  :: sz
real, dimension(1-g:sz(1)+g, 1-g:sz(3)+g, 1-g:sz(2)+g) :: s
real                                                   :: dh, rtime
real, dimension(3)                                     :: org
character*20                                           :: fname
...
write (22) dh, dh, dh
write (22) nn, rtime
write (22) (((s(i,k,j),i=1,ix),j=1,jx),k=1,kx)
close (unit=22)

return
end subroutine fileout_ikj
...
!> *******************************
subroutine exact_ikj (sz, g, e, dh, org)
implicit none
include 'cz_fparam.fi'
integer                                                ::  i, j, k, ix, jx, kx, g
integer, dimension(3)                                  ::  sz
real, dimension(3)                                     ::  org
real, dimension(1-g:sz(1)+g, 1-g:sz(3)+g, 1-g:sz(2)+g) ::  e
real                                                   ::  dh, pi, r2, x, y, z
...
do i=1, ix
  ...
  y = org(2) + dh*real(j-1)
  z = org(3) + dh*real(k-1)
  e(i,k,j) = sin(pi*x)*sin(pi*y) / sinh(r2*pi) * ( sinh(r2*pi*z)-sinh(r2*pi*(z-1.0)) )
end do
end do
end do
!$OMP END PARALLEL DO
...
!> *******************************
subroutine err_ikj (sz, idx, g, d, p, e, loc)
implicit none
integer                                                ::  i, j, k, ix, jx, kx, g
integer, dimension(3)                                  ::  sz, loc
integer, dimension(0:5)                                ::  idx
real, dimension(1-g:sz(1)+g, 1-g:sz(3)+g, 1-g:sz(2)+g) ::  p, e
real                                                   ::  r
double precision                                       ::  d, q
